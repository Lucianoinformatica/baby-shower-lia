...
const toggleMusic = () => {
  ...
  if (!entered) {
    return (
      ...
      <main className="relative min-h-screen overflow-hidden animate-cinematicEntrance bg-[radial-gradient(circle_at_top,#FFFDFB,#FFF4EE,#FDF0E8)] flex items-center justify-center px-6">
...
>
  ...
  {section === "lia" && <Lia guestName={guestName} />}
      ...
      {section !== "itinerario" && (
        ...
        <button
        onClick={toggleMusic}
        className="fixed top-5 right-5 z-50 w-14 h-14 rounded-full bg-white/35 backdrop-blur-[20px] border border-white/50 shadow-[0_10px_30px_rgba(212,184,167,0.25)] flex items-center justify-center text-2xl active:scale-95 transition group"
      >
        {musicPlaying ? "🎵" : "🔇"}
  ...
  <div className="absolute inset-0 rounded-[34px] bg-gradient-to-br from-white/25 via-transparent to-[#F7D7C4]/10 pointer-events-none" />
    {[
      ...
      <button
        ...
        className={`flex flex-col items-center gap-[2px] transition-all duration-500 ease-out px-3 py-2 rounded-[26px] active:scale-95 relative overflow-hidden ${
          section === id
...
<div className="absolute bottom-5 right-7 text-3xl opacity-60 animate-floatSlow">
  ...
  className="mb-8 flex items-center gap-3 text-sm text-[#A07C74] bg-white/60 backdrop-blur-2xl border border-white/50 rounded-full px-4 py-3 shadow-[0_10px_30px_rgba(212,184,167,0.18)] transition-all duration-300 hover:scale-[1.03] hover:bg-white/80 active:scale-95"
>
    ...
    La carita de Lia ya empieza a aparecer 😭 Votá a quien se parecerá más !
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className={`rounded-[28px] p-4 border transition active:scale-95 ${
              vote === "mama"
...
<div className="mt-8 bg-white/40 rounded-[30px] p-5 border border-white/50">
  ...
  <button
    ...
    className="mt-4 w-full rounded-full bg-[#F7D7C4] py-3 text-[#7A5C58] font-medium disabled:opacity-40 active:scale-95 transition"
  >
    Enviar predicción ✨
  </button>
...
<div className="mb-4">
  ...
  <div className="grid grid-cols-2 gap-3">
    <button
      ...
      className={`rounded-[22px] p-3 border transition active:scale-95 ${
